...
;;(add-to-list 'load-path/ "~/notebook/emacs/dev/muse")
;;(add-to-list 'load-path/ "~/notebook/emacs/dev/muse/lisp")
...
(setq muse-project-alist
      `(
        ("WikiPlanner"
         (
	,@(muse-project-alist-dirs "~/notebook/Plans/daily")
	,@(muse-project-alist-dirs "~/notebook/Plans/personal-near")
	,@(muse-project-alist-dirs "~/notebook/Plans/personal-far")
	,@(muse-project-alist-dirs "~/notebook/Plans/personal")
	,@(muse-project-alist-dirs "~/notebook/Plans/project")
	,@(muse-project-alist-dirs "~/quick/sslvpn/docs")
	,@(muse-project-alist-dirs "~/quick/office/isec")
	,@(muse-project-alist-dirs "~/quick/southpaw/docs")
	,@(muse-project-alist-dirs "~/quick/bm/docs")
	,@(muse-project-alist-dirs "~/notebook/Plans/older")
            :default "index" 
            :major-mode planner-mode
            :visit-link planner-visit-link)

         ;; pages from near-term and far-term personal get published to the
         ;; same directory (~/notebook/wiki/personal/) in XHTML form
         (:base "planner-html"
                ...
                ;; to a Planner page
                :include "/personal-?\\(near\\|far\\)?/"
                :path "~/notebook/wiki/personal")
         ;; pages from project plans
         ;; same directory (~/notebook/wiki/project/) in XHTML form
         (:base "planner-html"
                :header "~/notebook/wiki/.header.muse"
                :footer "<? include 'include/footer.inc.php' ?>"
                :before sacha/planner-before-markup
         ...
         (:base "planner-html"
                ...
                :include "/project/"
                :path "~/notebook/wiki/office")
         ;; pages from others will be published to stuff
         ;; same directory (~/notebook/wiki/project/) in XHTML form
         (:base "planner-html"
                :header "~/notebook/wiki/.header.muse"
                :footer "<? include 'include/footer.inc.php' ?>"
                ...
                :suffix ".php"
                :osuffix ".php"
                :exclude "/\\(personal-?\\(far\\|near\\)?\\|project\\)"
                :path "~/notebook/wiki/")
 	)
        )
      )
...
(setq planner-publishing-directory "~/public_html/notebook/wiki")
...
(setq planner-cyclic-diary-file "~/notebook/personal/diary.cyclic-tasks")
(setq planner-cyclic-diary-nag nil) ;; dont frustrate me for cyclic tasks
...
(setq planner-appt-task-use-appointments-section-flag t) ;; copy the appts to schedule section
;;;_- schedule
...
;             (split-string date "" t) ""))
             (split-string date "") "")) ;; bug fix for nld 10
