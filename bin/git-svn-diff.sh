...
git diff --no-prefix $(git rev-list --date-order --max-count=1 $TRACKING_BRANCH) $* | sed -e "/--- \/dev\/null/{ N; s|^--- /dev/null\n+++ \(.*\)|---\1	(revision 0)\n+++\1	(revision 0)|;}" -e "s/^--- .*/&	(revision $REV)/" -e "s/^+++ .*/&	(working copy)/" -e "s/^diff --git [^[:space:]]*/Index:/" -e "s/^index.*/===================================================================/"
